...
export class GenerateChoreWeekController implements BaseController {
  constructor(private readonly service: ChoreService) {}

  async run(req: Request, res: Response, next: NextFunction): Promise<void> {
    void next;
    const result = await this.service.generateWeek(req.params.householdId as string, req.body);
    res.status(201).json(choreWeekResponse(result));
  }
}
